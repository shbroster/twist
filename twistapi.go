...
// NewWithConfig creates a 'twist' with custom delimiters.
func NewWithConfig(s string, config TwistConfig) (twist, error) {
	if config.Delimiters[0] == config.Delimiters[1] {
		return twist{}, fmt.Errorf("delimeters must not match: %w", ErrInvalidConfig)
	}
	if len(config.Delimiters[0]) < 1 || len(config.Delimiters[1]) < 1 {
		return twist{}, fmt.Errorf("delimeters must not be empty: %w", ErrInvalidConfig)
	}
